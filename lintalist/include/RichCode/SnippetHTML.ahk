...
HighlightSnippet(Settings, ByRef Code, RTFHeader:="") ; based on HTML
{
	static Needle := "
	( LTrim Join Comments
		ODims)
		(\<\!--.*?--\>)       ; Multiline comments
;		|(<(?:\/\s*)?)(\w+)   ; Tags - original
		|([\[<]+(?:\/\s*)?)(\w+) ; Tags - adding [ for plugins
;		|([<>\/])             ; Punctuation - original
		|([<>\/\]])           ; Punctuation - added ] for plugins
		|(&[#\w]+?;)          ; Entities
;		|((?<=[>;])[^<>&]+)   ; Text - original
		|((?<=[>])[^<>&]+)    ; Text - comment is common in regular text: list; item; 
;		|(""[^""]*""|'[^']*') ; Strings ; commented as quoted strings are common in regular text: I'll etc
		|(\^\|)               ; Strings ; used for ^| caret plugin
;		|(\w+\s*)(=)          ; Attributes - original
;		|(\w+\s*=)            ; Attributes - alternative if need be
	)"
